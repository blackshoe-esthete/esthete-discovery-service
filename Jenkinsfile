pipeline {
    ...
    environment {
        // 환경 변수 설정
        IMAGE_NAME = 'jaehui327/esthete-discovery-service'
        DOCKERHUB_CREDENTIALS = credentials('dockerhub')
        JOB_NAME = 'esthete-discovery-service'
        ...
        stage('Update values.yaml on GitHub') {
            steps {
                script {
                    ...
                    def filePath = 'esthete-charts/esthete-discovery-chart/values.yaml'
...
image:
  repository: jaehui327/esthete-discovery-service
  tag: \"${env.IMAGE_TAG}\"
